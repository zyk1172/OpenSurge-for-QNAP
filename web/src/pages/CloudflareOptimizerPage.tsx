...
export function CloudflareOptimizerPage() {
  ...
  if (!draft || !data) return <div className="cloudflare-page">
    ...
    <section className="section mp-card" aria-busy="true">
      <div className="section-heading"><div><small>INITIALIZING</small><h2>正在连接 Cloudflare 优选服务</h2><p>读取目标域名、测速预算和最近一次优选结果。</p></div></div>
      {error ? <div className="error-banner" role="alert"><span>!</span><p>{error}</p><button type="button" onClick={() => void load()}>重试</button></div> : <div className="empty-state"><p>正在加载…</p></div>}
    </section>
  </div>
  ...
  return <div className="cloudflare-page">
    ...
    <section className="section">
      ...
      <div className="cloudflare-target-list">
        ...
        {draft.targets.map((target, index) => <div className="source-card cloudflare-target-card" key={`${index}-${target.domain}`}>

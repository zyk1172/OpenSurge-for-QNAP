...
export const api = {
  ...
  exportSourceSnapshot: (id: string) => request<SourceSnapshotFile>(`/api/v1/sources/${encodeURIComponent(id)}/export`, { method: 'POST' }),
	...
	saveHostHostsSync:(settings:Partial<Pick<HostHostsSync,'enabled'|'auto_update'|'interval_minutes'>>)=>request<HostHostsSync>('/api/v1/host-hosts-sync',{method:'PUT',body:JSON.stringify(settings)}),

...
describe('ProfileOverlayPanel', () => {
  beforeEach(() => {
    ...
    vi.mocked(api.syncHostHostsNow).mockResolvedValue({ ...hostSync, enabled: true })
  })
  ...
  it('updates host sync controls with patch-only payloads', async () => {
    ...
    await userEvent.click(await screen.findByRole('switch', { name: '启用宿主 Hosts' }))
    await waitFor(() => expect(api.saveHostHostsSync).toHaveBeenCalledWith({ enabled: true }))

    await userEvent.click(screen.getByRole('switch', { name: '定时更新' }))

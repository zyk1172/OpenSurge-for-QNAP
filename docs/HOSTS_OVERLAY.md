...
QNAP deployments mount the host `/etc/hosts` read-only at `/run/opensurge/host-hosts`; OpenSurge never replaces the container's own `/etc/hosts`. This is a container-creation setting: existing containers upgraded from an older release must add `/etc/hosts:/run/opensurge/host-hosts:ro` and recreate the container while keeping the existing `/data` mount. The advanced overlay exposes enable, scheduled refresh, interval and manual refresh controls. Automatic refresh checks every minute and only writes after the configured interval when the mapped file digest changes.

...
- Cloudflare 的普通用户配置按“域名、持续监控、强制刷新、测速模式”四个用户概念组织；域名使用一个多行编辑器，一行一个，可整批粘贴，空行忽略并自动去重；
- 持续监控只暴露检查间隔、延迟阈值和最大丢包，明确区分“当前 IP 轻量检查”和“完整优选”；质量下降时自动触发完整优选；
- 测速模式仍由 preset 管理候选数、并发、超时和下载预算，但页面使用只读表格展示当前 preset 的关键实际参数，既可核对行为又不把底层实现变成可任意组合的工程表单；
